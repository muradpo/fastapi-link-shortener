...
Для запуска всех тестов используется `pytest`.
...
Для проверки покрытия кода тестами используется `coverage`.

Сначала запускаем тесты с подсчетом покрытия:

coverage run -m pytest tests

Посмотреть процент покрытия:
...
В нем отображается визуальный отчет покрытия тестами.

Текущее покрытие проекта: **93%**.
...
После запуска необходимо открыть веб-интерфейс Locust:

http://localhost:8089

В интерфейсе указать:
...
После запуска можно наблюдать статистику количества запросов, времени ответа и ошибок.
